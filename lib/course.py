...
class Course(default.Notebook):
    ...
    def __get_lectures(self):
        ...
        __lectures = [list(reversed([lec_obj.option_showup for lec_obj in lectures])),
                      natsorted([lec_stem for lec_stem in lecture_file_stems], reverse=True),
                      natsorted([f'{lec_stem}.tex' for lec_stem in lecture_file_stems], reverse=True),
                      list(reversed([lec_obj.option_showup for lec_obj in lectures_number_only])),
                      natsorted([lec_stem for lec_stem in lecture_file_stems_number_only], reverse=True),
                      natsorted([f'{lec_stem}.tex' for lec_stem in lecture_file_stems_number_only], reverse=True),
                      lectures, lectures_number_only]
        return __lectures
    ...
    def __choose_to_compile(self):
        multi_select_compile = Rofi('multi-select-compile', self.__lectures[3], multi_select=True)
        multi_select_compile.run()
        if multi_select_compile.output['selections'] != '':
            selections = [list(reversed(self.__lectures[3])).index(selection) + 1
    ...
    def __parse_compile_args(self, options):
        if options == 'All':
            compiles = [self._parse_lecture_spec(lec) for lec in list(reversed(self.__lectures[4]))]
            return compiles
        elif options == 'Latest':
            return [self._parse_lecture_spec(self.__lectures[4][0])]
        elif options == 'Latest Two':
            return [self._parse_lecture_spec(lec) for lec in list(reversed(self.__lectures[4][:2]))]
    ...
    def compile_option(self, option=None):
        ...
        if select_compile.output['selections'] != '':
            self.__compiles = self.__parse_compile_args(select_compile.output['selections'])
    ...
    def edit_lecture(self):
        ...
        select_lecture.run()
        if select_lecture.output['selections'] != '' and select_lecture.output['index'] != -1:
    ...
    def new_lecture(self, filename=None):
        if filename is None:
            ...
            (self.path / new_lecture_file).write_text(f'\\{self.lecture_sign}{{{new_lecture_num}}}{{{self.date}}}')
        else:
            new_lecture_file = f'{filename}.tex'
            self.path.touch(new_lecture_file)
            (self.path / new_lecture_file).write_text(f'\\{self.lecture_sign}{{{filename}}}{{{self.date}}}')
        self.__init__()
    ...
    def remove_lecture(self, file=None):
        ...
        if remove_lecture.output['selections'] != '':
            os.remove(str(self.path / self.__lectures[2][remove_lecture.output['index']]))
